...
import { CONTRACT_ADDRESSES_V1, MarketId, MARKET_CONFIG_V1 } from '@/utils/contractConfig';
...
export const useComptrollerContract = (): ComptrollerContractHook => {
  ...
  const sendComptrollerTransaction = useCallback(
    async (methodName: string, args: any[]): Promise<TransactionResult> => {
      try {
        ...
        const transaction = {
          from: account,
          to: CONTRACT_ADDRESSES_V1.Comptroller,
  ...
  const getAllMarkets = useCallback(async (): Promise<string[]> => {
    try {
      const contract = await getContract(CONTRACT_ADDRESSES_V1.Comptroller, COMPTROLLER_ABI, false);
    ...
    async (userAddress: string): Promise<MarketId[]> => {
      try {
        ...
        for (const cTokenAddress of assetsIn) {
          const marketEntry = Object.entries(MARKET_CONFIG_V1).find(
