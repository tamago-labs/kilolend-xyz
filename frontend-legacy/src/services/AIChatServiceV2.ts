/**
 * AI Chat Service V2
 * Simplified service for multi-chain AI chat with session_id hardcoded to 1
 */
...
export class TextProcessor {
  ...
  static finalizeText(text: string): string {
    ...
    return this.normalizeLineBreaks(cleanedText);
  }
}

export class AIChatServiceV2 {
  private readonly STREAM_ENDPOINT = 'https://api.kilolend.xyz/stream_master';
  private readonly MESSAGES_ENDPOINT = 'https://api.kilolend.xyz/messages';
  private readonly TIMEOUT_MS = 30000; // 30 seconds timeout
  private readonly apiKey: string;
  private readonly DEFAULT_SESSION_ID = 1; // Always use session 1

  constructor() {
    this.apiKey = process.env.NEXT_PUBLIC_API_KEY || '';
  }

  async streamChat(prompt: string, userAddress: string, chainId: number, response: StreamResponse): Promise<void> {
  ...
  async getMessages(userAddress: string, chainId: number): Promise<MessagesListResponse> {
    try {
      const response = await fetch(`${this.MESSAGES_ENDPOINT}/${userAddress}/${this.DEFAULT_SESSION_ID}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'X-Api-Key': this.apiKey,
  ...
  async deleteMessages(userAddress: string, chainId: number): Promise<{ message: string; deleted_count: number }> {
    try {
      const response = await fetch(`${this.MESSAGES_ENDPOINT}/${userAddress}/${this.DEFAULT_SESSION_ID}`, {
        method: 'DELETE',
        headers: {
...
export const aiChatServiceV2 = new AIChatServiceV2();