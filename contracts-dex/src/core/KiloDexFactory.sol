...
import {KiloDexPool} from './KiloDexPool.sol';

/// @title KiloDex factory
/// @notice Deploys KiloDex pools and manages control over government fees
contract KiloDexFactory is BaseSplitCodeFactory, IFactory {
  ...
  constructor(uint32 _vestingPeriod, address _poolOracle) BaseSplitCodeFactory(type(KiloDexPool).creationCode) {
    poolInitHash = keccak256(type(KiloDexPool).creationCode);
